...
Role: ANALYST
TicketId: T0015
SourceTicket: `.windsurf/sandbox/MAIL/ANALYST/IN/T0015_20260417-1758_ANALYST_synthesize-rule-architecture-from-evidence-pack.md`
...
4. Common-layer: `.aiassistant/rules/00_COMMON.md`.
...
flowchart TD
    ...
    C["Spec-layer<br/>.aiassistant/rules/specs/*"]
    D["Common-layer<br/>.aiassistant/rules/00_COMMON.md"]
...
- Ownership для `.windsurf/sandbox/00_STATE.md` и для staging process-артефактов.
...
- Разорвать bootstrap-cycle между `AGENTS.md` и `.aiassistant/rules/00_COMMON.md`.
...
- Закрепить ownership и expected update-flow для `.windsurf/sandbox/00_STATE.md`.
- Не выполнять массовую ретро-правку old `MAIL/**`.

## 9. Change-plan для CODER

1. Нормализовать bootstrap.
   Исправить `AGENTS.md`, `.aiassistant/rules/00_COMMON.md` и при необходимости `specs/*`, чтобы чтение было однонаправочным и без циклов.
...
6. Ввести physical skills-layer.
   Создать каркас `.aiassistant/skills` и перенести туда только reusable recipes, не затрагивая конституционные нормы.
7. Зафиксировать ownership process-state.
   Явно описать, кто и когда обновляет `.windsurf/sandbox/00_STATE.md`, и как staging process-артефактов входит в definition of done.
...
1. `.windsurf/sandbox/MAIL/ANALYST/OUT/T0015_20260417-1758_ANALYST_synthesize-rule-architecture-from-evidence-pack_file-pack.md`
   Это главный upload-ready документ с одной рекомендуемой архитектурой, bootstrap-flow, границами слоёв и change-plan.
2. `.windsurf/sandbox/MAIL/ANALYST/OUT/T0014_20260417-0220_ANALYST_audit-agent-rules-and-design-skill-extraction_report.md`
   Это краткая карта конфликтов, кандидатов и cleanup-направлений из первого прохода.
3. `.windsurf/sandbox/MAIL/ANALYST/OUT/T0014_20260417-0220_ANALYST_audit-agent-rules-and-design-skill-extraction_evidence-pack.md`
   Это компактный evidence appendix с точечными hotspot-ссылками и фактами по drift/conflicts.
4. `.windsurf/sandbox/MAIL/VERIFIER/OUT/T0016_20260417-1803_VERIFIER_validate-templates-against-rules-and-process_report.md`
